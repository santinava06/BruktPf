    ...
    const groups = await ExpenseGroup.findAll({
      include: [
        ...
        },
        {
          model: GroupMember,
          as: 'members',
          attributes: ['role', 'fecha_union'],
          include: [
            {
              model: User,
              as: 'user',
              attributes: ['id', 'email', 'nombre', 'apellido', 'username']
            }
          ]
        }
      ],
...
export const updateGroup = async (req, res) => {
  try {
    ...
    if (group.creator_id !== userId && (!membership || membership.role !== 'admin')) {
      return res.status(403).json({
        error: 'Solo el creador o administradores pueden editar el grupo'
      });
    }
...
export const removeMember = async (req, res) => {
  try {
    ...
    if (!membership || (membership.role !== 'admin' && group.creator_id !== userId)) {
      return res.status(403).json({
...
// Obtener permisos del usuario en un grupo
export const getUserPermissions = async (req, res) => {
  try {
    ...
    const permissions = {
      canAddExpenses: ['admin', 'moderator'].includes(membership.role),
      canEditExpenses: ['admin', 'moderator'].includes(membership.role),
      canDeleteExpenses: membership.role === 'admin',
      canManageMembers: membership.role === 'admin',
      canChangeRoles: membership.role === 'admin',
      canDeleteGroup: membership.role === 'admin'
    };
