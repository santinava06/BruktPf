...
export const getGoogleAccount = async (req, res) => {
  try {
    ...
    const user = await User.findByPk(userId, {
      attributes: ['id', 'google_id', 'email', 'nombre', 'apellido']
    });
    ...
    // Verificar si tiene cuenta Google asociada
    if (!user.google_id) {
      return res.json({
        hasGoogleAccount: false,
        message: 'No hay cuenta de Google asociada'
      });
    }

    // En una implementación real, aquí podríamos hacer una llamada a la API de Google
    // para obtener información adicional del perfil, pero por ahora devolvemos
    // la información básica que tenemos
    res.json({
      hasGoogleAccount: true,
      googleAccount: {
        id: user.google_id,
        email: user.email, // Asumimos que el email de Google es el mismo que el del usuario
        displayName: `${user.nombre} ${user.apellido}`
      }
    });

  } catch (error) {
    console.error('Error en getGoogleAccount:', error);
    res.status(500).json({
      error: 'Error interno del servidor',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
...
export const unlinkGoogleAccount = async (req, res) => {
  try {
    ...
    if (!user.google_id) {
      return res.status(400).json({ error: 'No hay cuenta de Google asociada' });
    }
    ...
    console.error('Error en unlinkGoogleAccount:', error);
    res.status(500).json({
      error: 'Error interno del servidor',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
